// Copied from packages/agents/stellar-oracle/src/horizon.ts. Only getAssetMetadata
// differs: the SDK pins @stellar/stellar-sdk 14, where AssetRecord exposes
// `accounts`/`balances` objects instead of the v12 `num_accounts`/`amount`.
...
export async function getAssetMetadata() {
  return getCached('asset:usdc-metadata', CACHE_TTLS.assetMs, async () => {
    ...
    return {
      code: USDC_CODE,
      issuer: USDC_ISSUER,
      accounts: record?.accounts?.authorized ?? 0,
      amount: record?.balances?.authorized ?? '0',
    };
  });
}
